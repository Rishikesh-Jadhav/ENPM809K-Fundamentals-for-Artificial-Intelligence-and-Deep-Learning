...
def dropout_forward(x, dropout_param):
    ...
    if mode == "train":
        # Generate a binary dropout mask to apply dropout
        mask = (np.random.rand(*x.shape) < p) / p 

        # Apply dropout to the input x by element-wise multiplication with the mask
        out = x * mask

    elif mode == "test":
        # In 'test' mode, return the input as is
        out = x

    # Cache the dropout parameters and mask for the backward pass
    cache = (dropout_param, mask)
    # Ensure the output has the same data type as x and avoid unnecessary copies
...
def dropout_backward(dout, cache):
    ...
    dx = None
    if mode == "train":
        # During 'train' mode, undo dropout by applying the mask to the upstream derivatives
        dx = mask * dout

    elif mode == "test":
        # In 'test' mode, simply pass the upstream derivatives as is
        dx = 1 * dout
    return dx
