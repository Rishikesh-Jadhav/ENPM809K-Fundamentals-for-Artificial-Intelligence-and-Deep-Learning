...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    cache = (x, prev_h, Wx, Wh, b, next_h) # Store values for the backward pass   
...
def rnn_step_backward(dnext_h, cache):
    ...
    dtanh = 1 - tanh**2                               # Local derivative of tanh
    dnext_tanh = dtanh * dnext_h                      # Gradient of tanh in the backward pass

    dx = dnext_tanh.dot(Wx.T)                         # Gradient of input data
    dprev_h = dnext_tanh.dot(Wh.T)                    # Gradient of previous hidden state
    dWx = (x.T).dot(dnext_tanh)                       # Gradient of input-to-hidden weights
    dWh = (prev_h.T).dot(dnext_tanh)                  # Gradient of hidden-to-hidden weights
    db = dnext_tanh.sum(axis=0)                       # Gradient of the bias vector
    db = db.T.flatten()                               # Flatten the bias gradient
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    # Initialize variables and containers for the hidden states and forward cache
    N, T, D = x.shape
    H = h0.shape[1] # or Wh.shape[0]
    h = np.zeros((N, T, H))
    forward_cache = {}

    # Loop through the sequence and compute the forward pass using rnn_step_forward
    for t in range(T):
        h[:, t, :], forward_cache[t] = rnn_step_forward(x[:, t, :], h0 if \
        t == 0 else h[:, t-1, :], Wx, Wh, b)

    # Store relevant information in the cache    
...
def rnn_backward(dh, cache):
    ...
    # Initialize variables and containers for gradients   
    N, T, H = dh.shape
    ...
    # Initialize the gradient of the previous hidden state to zero
    dprev_h = 0

    # Loop backward through the sequence and compute the gradients using rnn_step_backward
    ...
    # The final gradient of the initial hidden state is the gradient of the previous hidden state
    dh0 = dprev_h    
...
def word_embedding_forward(x, W):
    ...
    # Use array indexing to obtain word vectors for each word in the input sequence
...
def word_embedding_backward(dout, cache):
    ...
    # Use the np.add.at function to accumulate gradients for word embeddings
    np.add.at(dW, x, dout)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    # Gates
    a_i = a[:, 0*H:1*H]  # Input gate (N, H)
    a_f = a[:, 1*H:2*H]  # Forget gate (N, H)
    a_o = a[:, 2*H:3*H]  # Output gate (N, H)
    a_g = a[:, 3*H:4*H]  # Cell gate (N, H)
    
    # Activation functions
    i = sigmoid(a_i)  # Input gate activation (N, H)
    f = sigmoid(a_f)  # Forget gate activation (N, H)
    o = sigmoid(a_o)  # Output gate activation (N, H)
    g = np.tanh(a_g)  # Cell gate activation (N, H)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    # Gradients with respect to the output of each gate and cell state
    do = dnext_h * np.tanh(next_c)
    dc = (1 - np.tanh(next_c) ** 2) * dnext_h * o
    dc += dnext_c # Add gradients of the next cell state
    
    # Gradients with respect to the gates
    df = dc * prev_c
    dprev_c = dc * f
    di = g * dc
    dg = i * dc
