...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        positional_embeddings = self.positional_encoding(caption_embeddings)
        image_embeddings = self.visual_projection(features).unsqueeze(1)  # (N, 1, wordvec_dim)

        # (2) Masking
        tgt_mask = self.generate_tgt_mask(T).to(captions.device)

        # (3) Decoder
        combined_embeddings = positional_embeddings + image_embeddings
        transformer_output = self.transformer(combined_embeddings, tgt_mask=tgt_mask)
